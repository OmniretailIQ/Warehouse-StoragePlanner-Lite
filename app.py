...
Optional: upload **Bin Master** to get **Pick-Face/Bulk bin assignments** (Zone → Row → Bay → Level/Tier).
""")
...
def compute_minmax(set_df, set_type, festival_map, pf_policy_map, cov_min_days, cov_max_days, cap_total_pcs):
    ...
    # Capacity governor (piece ceiling) — compress Bulk across tiers
    def tier(a, r):
...
if run_btn:
    ...
    # -------------------- Display --------------------
    st.subheader("Colour Sets — Zoning & Min–Max")
    st.dataframe(colour_final.head(500))

    st.subheader("Size Sets — Zoning & Min–Max")
    ...
    def to_excel(col_df, size_df, pf_df=None, bulk_df=None):
        ...
        with pd.ExcelWriter(output, engine="xlsxwriter") as writer:
            pd.DataFrame({"Notes":[
                ...
                "Colour & Size sets with ABC/RRS roll-ups, Style Density, Zoning, festival-aware Min–Max, and capacity-governed totals.",
                "PF levels 1–3 (configurable); Zone=floor; Tier=floor(F00->1, F01->2...); Bin=loc_code_scan; capacity per slot configurable.",
